...
INSERT INTO `contacto` VALUES (1,_binary 'ºg\ny± ña€Øá\ÂU∑Z',_binary 'º•\“60o‘ùh1.\ı<{!',_binary 'Jcø´º\…\Ù\Òæ˝',1),(2,_binary '\n\÷\’aXzÖ\˜O6Y\Ô',_binary 'º•\“60o‘ùh1.\ı<{!',_binary '\€}\ŒCÅˇ\≈Y¢ˇ\ˆ&‚â¥ùë;\›˝üLÆ;R\ÔGpè',1);
...
INSERT INTO `cuentabacaria` VALUES (1,1,_binary 'ê\ı\Ù:áX,B™l\ﬂ\ﬁ\√d°∞ä\Í∫*V\ÿ\Ù\ÈU0pû~N{',1),(2,1,_binary '\œdq`.Ñm\√H!ß\È‘õ˙íP¨\‰c xç_\”',1);
...
INSERT INTO `persona` VALUES (1,_binary 'õ}\ƒ\—\Ëí\‚•j˙qP8N',_binary 'â∏\ÁCû˛íHXΩ\Û\˜I\„Ãä\Í∫*V\ÿ\Ù\ÈU0pû~N{',_binary 'w$\ÚD\‰\ÁrÄßl\⁄¯π>˛',_binary '\¬}–∫f8´KK\r\œU\—\≈',_binary 'º•\“60o‘ùh1.\ı<{!',_binary 'º•\“60o‘ùh1.\ı<{!',_binary '˘yrö#F¥≥°óDØ˙ã•',_binary 'º•\“60o‘ùh1.\ı<{!',_binary 'º•\“60o‘ùh1.\ı<{!',_binary '<*\0:?\È\’\‰c5{Ä',1),(2,_binary 'G∫\À¶õp™.\œ2ˇó\÷',_binary 'â∏\ÁCû˛íHXΩ\Û\˜I\„Ãºg\ny± ña€Øá\ÂU∑Z',_binary '\·W\Í¶fÉäP\ÚNÖ\À:m',_binary '\Ê6Ø⁄í\r\ ƒò4X}.æ\Ô4',_binary 'º•\“60o‘ùh1.\ı<{!',_binary 'º•\“60o‘ùh1.\ı<{!',_binary '˘yrö#F¥≥°óDØ˙ã•',_binary 'º•\“60o‘ùh1.\ı<{!',_binary 'º•\“60o‘ùh1.\ı<{!',_binary '<*\0:?\È\’\‰c5{Ä',1);
...
INSERT INTO `preguntapersona` VALUES (1,1,1,_binary '\¬\\1\\Ω°G\—E¢Øu!}û',1);
...
INSERT INTO `usuario` VALUES (1,_binary 'º•\“60o‘ùh1.\ı<{!',_binary '2\„<+\„[¢≥g•¶l ',_binary '\Òø¡\Ù»üp4´ü\‡˝V•õ@\0SN\råé≠9+ûÜ≤¥∞ä\Í∫*V\ÿ\Ù\ÈU0pû~N{',_binary 'º•\“60o‘ùh1.\ı<{!',_binary 'ì\nHXpåPXèP$\Ú“õ°~\”?õ\€;Œêz~õ˙¥;\r',1);
...
begin
	...
	concat('[',
        GROUP_CONCAT(
			...
			'idtipopersona',
			aes_decrypt (per_idTipoPrersona, llave),
			'idrol',
			aes_decrypt (per_idRol, llave),
			'activofintec',
			aes_decrypt (per_ActivoFintec, llave),
			'idregimenfiscal',
			aes_decrypt (per_RegimenFiscal, llave),
			'idcuentabanco',
			aes_decrypt (per_idCtaBanco, llave),
			'banco',
			c2.Alias,
			'clabe',
			aes_decrypt (c.b_CLABE,llave),
			'logo_persona',
			aes_decrypt (per_logo ,llave),
			'logo_usuario',
			aes_decrypt (u.u_imagenUsuario, llave)	) 
	separator ',')
    , ']')
    into salida
from
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `UpdateLlaveUsuario`(entrada blob,
llave varchar(100)) RETURNS blob
begin
	-- {"idUsuario" : "1","Llave" : "isra10"}
	declare resultado blob;
 ...
 JSON_OBJECT ('Perfil',
	p.p_idPerfil,
	'Persona',
	aes_decrypt(u.u_idPersona,llave),
	'Usuario',
	aes_decrypt (u.u_NombreUsuario,llave)) 
 into
	resultado
 from
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `ValidarLlave`(entrada blob,
llave varchar(100)) RETURNS blob
begin
  declare resultado blob;
...
-- Dump completed on 2023-09-19 12:08:20

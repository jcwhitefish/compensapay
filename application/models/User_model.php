...
//	private string $dbprod = 'compensapay';
		...
		public function getUserByFintechClabe ( string $clabe, string $env = NULL ) {
			//Se declara el ambiente a utilizar
			$this->enviroment = $env === NULL ? $this->enviroment : $env;
			$this->base = strtoupper ( $this->enviroment ) === 'SANDBOX' ? $this->dbsandbox : $this->dbprod;
			//Se genera el query para obtener datos
			$query = "SELECT t1.id, t1.user, t1.name, t1.last_name, t1.email, t2.short_name
FROM compensatest_base.users t1
FROM $this->base.users t1
	INNER JOIN $this->base.companies t2 ON t2.id = t1.id_company
	INNER JOIN $this->base.fintech t3 ON t3.companie_id = t2.id
WHERE t1.profile = 0 AND t3.arteria_clabe = '$clabe' LIMIT 1";

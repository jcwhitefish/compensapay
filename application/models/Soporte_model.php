...
class Soporte_model extends CI_Model{
	...
	public function getModules(){
		$items = [];
		$query = "SELECT tcm_id, tcm_module FROM compensatest_base.tck_module ORDER BY tcm_id";
	...
	public function getTopic(int $arg, ?string $env ='SANDBOX'){
		$items = [];
		$query = "SELECT tct_id, tct_topic FROM compensatest_base.tck_topic WHERE id_module = '{$arg}' order by tct_id";
		if ($result = $this->db->query($query)) {
			if ($result->num_rows() > 0) {
				foreach ($result->result_array() as $row){
					$item=[
	...
	public function newTicket (array $args, ?string $env = 'SANDBOX'){
		...
		$query = "INSERT INTO compensatest_base.tck_ticket (id_topic, id_companie, tck_folio, tck_issue, tck_description, tck_status) 
            VALUES ('{$args['topic']}', '{$args['companie']}', '{$folio}','{$args['issue']}', '{$args['description']}', 
                    '{$args['status']}')";
		if ($result = $this->db->query($query)){
			$id = $this->db->insert_id();
			$query = "INSERT INTO compensatest_base.tck_tracking(id_ticket, tcs_status, tcs_message, tcs_flow) 
	...
	public function getTicketsFromCompanie(int $companie){
		...
		$query = "SELECT tck_folio, tck_issue, tck_status, tck_description, tck_created_at 
					FROM compensatest_base.tck_ticket 
	...
	public function getTickettraking(int $idTicket){
		$tickets = [];
		$query = "SELECT t1.tcs_status, t1.tcs_message, t1.tcs_flow FROM compensatest_base.tck_tracking t1 
					inner JOIN compensatest_base.tck_ticket t2 ON t2.tck_id = t1.id_ticket 
	...
	public function getIdTicketByFolio($folio) {
		$query = "SELECT tck_id FROM compensatest_base.tck_ticket WHERE tck_folio = '{$folio}'";
        if ($result = $this->db->query($query)) {
            if ($result->num_rows() > 0) {
				foreach ($result->result_array() as $row){
                    $idTicket = $row['tck_id'];
	...
	public function newMssUser(array $args,?string $env = 'SANDBOX'){
		...
		if ($this->db->query($query)){
			$query = "UPDATE compensatest_base.tck_ticket SET tck_status = 2 WHERE tck_id = '{$id}'";
			if ($this->db->query($query)){

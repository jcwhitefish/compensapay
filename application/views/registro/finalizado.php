<div class="p-5">
    <div class="card esquinasRedondas">
        <div class="card-content especial-p">
            <p>Hola $var_nombre, bienvenido a <strong>Compensa</strong> pay, hemos enviado un correo a $var_correo, por favor activa tu cuenta para poder ingresar a nuestra plataforma.</p><br>
            <p>Si tienes dudas o comentarios puedes comunicarte al correo <strong>soporte@compensapay.mx</strong></p>
            <div class="center-align">
                <img src="<?= base_url('assets/images/CompensaPay_Logos-02.png'); ?>" alt="Logo" class="size-image">
            </div>
        </div>
    </div>
</div>

<style>
    .size-image{
        max-width: 40%; 
        max-height: 40%; 
        width: auto; 
        height: auto;
    }
    .especial-p {
        padding: 5% !important;
    }

</style>
